...
hold on;
unit_membrane_mesh_length = actin_mesh_min_length/3;
...
volume_constraint_coefficient = 6;
V_initial = 4/3*pi*15^3;
k_a = 1;


type_list =       [0,      1     2      3      4];
type_Co =         [-1/10,  0     0     1/5    1/4];
type_rigidities = [1.5,      1.5     1.5      1.5     1.5];
type_gaussian_rigidities = [1.5,      1.5     1.5      1.5     1.5];
...
[membrane_mesh_list] = assign_type_properties(membrane_mesh_list,type_list,type_Co,type_rigidities,type_gaussian_rigidities);
...
for df = 0:10000
    ...
    view([90,0]);xlim([0,18]);ylim([-20,20]);zlim([-20,20]);
...
%     imwrite(current_frame.cdata,'EnergyPlot.tif','WriteMode','append');
    if df == 0
       ...
       kick_displacement = 0.5*(area_reduced/pi)^(.5);
    end
    
    %% checking mesh condition
    if mod(df,3) == 0
%         hold on ;trisurf(membrane_mesh,'FaceColor','red','FaceAlpha',.3,'EdgeAlpha',0.2);daspect([1,1,1]);hold off;
        h4 = figure(4);
        close(h4)
        all_IDs = extract_IDs(diffused_membrane_mesh_list);
        for type_ind = 1:no_of_types
        ...
        [diffused_membrane_mesh_list] = ...
                enhance_structures_based_on_curvature(diffused_membrane_mesh_list,kick_displacement/2,surface_tension,hardball_rad,100,pressure,temperature,volume_constraint_coefficient,V_initial);
    end
    ...
    [diffused_membrane_mesh_list,acceptance_rate_lateralDiffusion] = diffuse_vertex_flip(diffused_membrane_mesh_list,temperature,hardball_rad,pressure,surface_tension,area_reduced,k_a);
